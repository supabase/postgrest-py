...
from postgrest_py.utils import SyncClient, sanitize_param, sanitize_pattern_param

CountMethod = Union[Literal["exact"], Literal["planned"], Literal["estimated"]]


class SyncRequestBuilder:
    def __init__(self, session: SyncClient, path: str):
    ...
    def select(self, *columns: str, count: Optional[CountMethod] = None):
        ...
        return SyncSelectRequestBuilder(self.session, self.path, method, {})
    ...
    def insert(self, json: dict, *, count: Optional[CountMethod] = None, upsert=False):
        ...
        return SyncQueryRequestBuilder(self.session, self.path, "POST", json)
    ...
    def update(self, json: dict, *, count: Optional[CountMethod] = None):
        ...
        if count:
            prefer_headers.append(f"count={count}")
        self.session.headers["prefer"] = ",".join(prefer_headers)
        return SyncFilterRequestBuilder(self.session, self.path, "PATCH", json)
    ...
    def delete(self, *, count: Optional[CountMethod] = None):
        ...
        if count:
            prefer_headers.append(f"count={count}")
        self.session.headers["prefer"] = ",".join(prefer_headers)
        return SyncFilterRequestBuilder(self.session, self.path, "DELETE", {})


class SyncQueryRequestBuilder:
    def __init__(self, session: SyncClient, path: str, http_method: str, json: dict):
        self.session = session
        self.path = path
    ...
    def execute(self) -> Tuple[Any, Optional[int]]:
        r = self.session.request(self.http_method, self.path, json=self.json)
...
class SyncFilterRequestBuilder(SyncQueryRequestBuilder):
    def __init__(self, session: SyncClient, path: str, http_method: str, json: dict):
    ...
    def cs(self, column: str, values: Iterable[str]):
        values = map(sanitize_param, values)
        values = ",".join(values)
        return self.filter(column, "cs", '{values}')

    def cd(self, column: str, values: Iterable[str]):
        values = map(sanitize_param, values)
        values = ",".join(values)
        return self.filter(column, "cd", '{values}')

    def ov(self, column: str, values: Iterable[str]):
        values = map(sanitize_param, values)
        values = ",".join(values)
        return self.filter(column, "ov", '{values}')
...
class SyncSelectRequestBuilder(SyncFilterRequestBuilder):
    def __init__(self, session: SyncClient, path: str, http_method: str, json: dict):
        super().__init__(session, path, http_method, json)
